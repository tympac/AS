...
$udts_cap_modules = \cmsms()->GetModuleOperations()->get_modules_with_capability('udts');
$udts_handlers_list = [];

if( !formbuilder_utils::is_CMS2_3() )
{
  $udts_handlers_list['core_udts'] = $this->lang('core_udts');
  $udt_handler_pref = $this->GetPreference('udt_handler_pref', 'core_udts');
}
else
{
  $udts_handlers_list['core_sps'] = $this->lang('core_simple_plugins');
  $udt_handler_pref = $this->GetPreference('udt_handler_pref', 'core_sps');
}

foreach($udts_cap_modules as $mod_name)
{
  $udts_handlers_list[$mod_name] = \cms_utils::get_module($mod_name)->GetFriendlyName();
}
		...
		if ($this->CheckPermission('Modify Forms'))
			{
				...
				$oneset->xml = $this->CreateLink($id,'exportxml','',"<img src=\"".$this->GetModuleURLPath()."/images/xml_rss.gif\" class=\"systemicon\" alt=\"Export Form as XML\" />",array('form_id'=>$thisForm['form_id']));
...
$smarty->assign('udts_handlers_list', $udts_handlers_list);
$smarty->assign('udt_handler_pref', $udt_handler_pref);
